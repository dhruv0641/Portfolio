...
router.get('/', (req, res) => {
  ...
  const all = db.tools.getAll(tenantId);
...
router.get('/:id', (req, res) => {
  const item = db.tools.getById(req.params.id);
...
router.post('/', authenticate, validate(toolCreateSchema), asyncHandler(async (req, res) => {
  ...
  const newItem = db.tools.create(req.validatedBody, tenantId);
...
router.put('/:id', authenticate, validate(toolUpdateSchema), asyncHandler(async (req, res) => {
  const meta = getAuditMeta(req);
  const updated = db.tools.update(req.params.id, req.validatedBody);
...
router.delete('/:id', authenticate, asyncHandler(async (req, res) => {
  const meta = getAuditMeta(req);
  const deleted = db.tools.delete(req.params.id);
