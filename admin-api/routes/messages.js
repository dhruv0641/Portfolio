...
router.post('/', validate(messageCreateSchema), asyncHandler(async (req, res) => {
  ...
  const encryptedData = {
    name: data.name,
    email: isEnabled('data_encryption') ? encrypt(data.email) : data.email,
    subject: data.subject || '',
    message: isEnabled('data_encryption') ? encrypt(data.message) : data.message,
    status: 'unread',
  };
...
router.patch('/:id/read', authenticate, asyncHandler(async (req, res) => {
  ...
  const updated = db.messages.update(req.params.id, { status: 'read' });
...
router.delete('/:id', authenticate, asyncHandler(async (req, res) => {
  const meta = getAuditMeta(req);
  const deleted = db.messages.delete(req.params.id);
